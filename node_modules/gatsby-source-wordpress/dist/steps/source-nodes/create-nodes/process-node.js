...
var _publicUrlResolver = require("gatsby-plugin-utils/dist/polyfill-remote-file/graphql/public-url-resolver");
    ...
    const htmlMatchesWithImageResizes = await Promise.all(imgTagMatches.map(async ({
      ...
      try {
        if (gatsbyTransformerSharpSupportsThisFileType) {
          ...
          publicUrl = (0, _publicUrlResolver.publicUrlResolver)({
            url: imageUrl,
            mimeType: imageNode.mimeType,
...
const replaceFileLinks = async ({
  ...
  const hrefMatches = [// match url pathnames in html fields, for ex /wp-content/uploads/2019/01/image.jpg
  ...((0, _execall.default)(/(\\"|\\'|\()([^'"()]*)(\/wp-content\/uploads\/[^'">()]+)(\\"|\\'|>|\))/gm, nodeString) || []), // match full urls in json fields, for ex https://example.com/wp-content/uploads/2019/01/image.jpg
